---
name: broadcast
description: Translates one mature Economic Model or Alignment document into external-facing copy in outcomes/broadcast. Use for the BROADCAST phase once a verified model or alignment doc exists.
---
...
This is the BROADCAST phase. Argument: a filename in `<eng>/outcomes/economic_models/` or
`<eng>/outcomes/alignment/`.

1. Verify the target file exists. Check `<eng>/outcomes/verification/` for its verification
...
4. Write the broadcaster's returned Markdown verbatim to
   `<eng>/outcomes/broadcast/post--<slug>.md` using the Write tool. (The gate hook validates the
